...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$STAG_ROOT"/vendor/stag/build/tools/smali/baksmali.jar
        export SMALIJAR="$STAG_ROOT"/vendor/stag/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$STAG_ROOT"/vendor/stag/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$STAG_ROOT"/vendor/stag/build/tools/"$HOST"/compact_dex_converter
    fi
