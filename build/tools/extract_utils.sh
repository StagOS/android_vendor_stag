...
function setup_vendor() {
    ...
    export STAG_ROOT="$3"
    if [ ! -d "$STAG_ROOT" ]; then
        echo "\$STAG_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$STAG_ROOT/$OUTDIR" ]; then
        mkdir -p "$STAG_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$STAG_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$STAG_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$STAG_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$STAG_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$STAG_ROOT"/prebuilts/tools-stag/common/smali/baksmali.jar
        export SMALIJAR="$STAG_ROOT"/prebuilts/tools-stag/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$STAG_ROOT"/prebuilts/tools-stag/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$STAG_ROOT"/prebuilts/tools-stag/"${HOST,,}"-x86/bin/compact_dex_converter
>>>>>>> 8194b8e... extract_utils: Source the prebuilts from the new repo
    fi
    ...
    if [ ! -f "$STAG_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$STAG_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$STAG_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$STAG_ROOT"/vendor/stag/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$STAG_ROOT"/"$OUTDIR"/radio
